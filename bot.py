    ...
    async with db_pool.acquire() as conn:
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS users (
                ...
                price_list TEXT DEFAULT '{"all": 90}',
...
async def get_code_from_telegram(phone: str, session_string: str, is_2fa: bool = False, password_2fa: str = ""):
    try:
        ...
        for dialog in dialogs:
            try:
                messages = await client.get_messages(dialog.id, limit=30)
...
def get_price_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(
                text="Изменить цену страны",
                callback_data="change_country_price",
                icon_custom_emoji_id="5904462880941545555"
            )
        ],
        [
...
async def process_work_code(message: Message, state: FSMContext):
    ...
    try:
        ...
        async with db_pool.acquire() as conn:
            await conn.execute(
                "INSERT INTO work_accounts (user_id, phone, session_string, is_2fa) VALUES ($1, $2, $3, $4) ON CONFLICT (user_id, phone) DO UPDATE SET session_string = $3",
...
async def process_work_2fa(message: Message, state: FSMContext):
    ...
    try:
        ...
        async with db_pool.acquire() as conn:
            await conn.execute(
                "INSERT INTO work_accounts (user_id, phone, session_string, is_2fa) VALUES ($1, $2, $3, $4) ON CONFLICT (user_id, phone) DO UPDATE SET session_string = $3, is_2fa = $4",
...
async def process_sale_code(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            '<b><tg-emoji emoji-id="5870753782874246579">✍</tg-emoji> Введите страну аккаунта на русском (например: Россия, Казахстан, Украина):</b>',
            parse_mode=ParseMode.HTML
        )
...
async def process_sale_country(message: Message, state: FSMContext):
    ...
    try:
        ...
        user = await get_user(user_id)
        price_list = json.loads(user['price_list']) if user and user['price_list'] else {"all": 90}
        price = price_list.get(country, price_list.get("all", 90))
        
        async with db_pool.acquire() as conn:
            await conn.execute(
                "INSERT INTO sale_accounts (user_id, phone, session_string, country, is_2fa, price_rub) VALUES ($1, $2, $3, $4, $5, $6) ON CONFLICT (user_id, phone) DO UPDATE SET session_string = $3, country = $4, is_2fa = $5",
                user_id, phone, session_string, country, is_2fa, price
            )
...
async def edit_price(callback: CallbackQuery):
    ...
    text = '<b><tg-emoji emoji-id="5904462880941545555">🪙</tg-emoji> Текущий прайс-лист:</b>\n\n'
    for country, price in price_list.items():
        if country == "all":
            text += f"<b>Общая цена:</b> {price}₽\n"
        else:
            text += f"<b>{country}:</b> {price}₽\n"
    
    text += '\n<i>Для изменения цены страны введите:</i>\n<code>Страна: Цена</code>\n<i>Пример: Россия: 150</i>'
...
@dp.callback_query(F.data == "change_country_price")
async def change_country_price(callback: CallbackQuery, state: FSMContext):
    await callback.message.answer(
        '<b><tg-emoji emoji-id="5870753782874246579">✍</tg-emoji> Введите страну и цену в формате:</b>\n'
        '<code>Россия: 150</code>\n\n'
        '<i>Или введите "all: 100" для изменения общей цены</i>',
        parse_mode=ParseMode.HTML,
        reply_markup=get_back_keyboard()
    )
    await state.set_state(PriceStates.waiting_country_price)
    await callback.answer()


@dp.message(PriceStates.waiting_country_price)
async def save_country_price(message: Message, state: FSMContext):
    user_id = message.from_user.id
    user = await get_user(user_id)
    ...
    try:
        ...
        country = parts[0].strip().capitalize()
        price = float(parts[1].strip())
        
        import json
        price_list = json.loads(user['price_list']) if user and user['price_list'] else {"all": 90}
        price_list[country] = price
        
        async with db_pool.acquire() as conn:
            await conn.execute(
                "UPDATE users SET price_list = $1 WHERE user_id = $2",
                json.dumps(price_list), user_id
            )
        
        await message.answer(
            f'<b><tg-emoji emoji-id="5870633910337015697">✅</tg-emoji> Цена для {country} установлена: {price}₽</b>',
            parse_mode=ParseMode.HTML,
            reply_markup=get_main_keyboard()
        )
        await state.clear()
        ...
        await message.answer(
            f'<tg-emoji emoji-id="5870657884844462243">❌</tg-emoji> Ошибка формата. Используйте: Страна: Цена',
            parse_mode=ParseMode.HTML
        )
...
async def handle_custom_command(message: Message):
    ...
    # Проверяем, есть ли у отправителя рабочие аккаунты
    async with db_pool.acquire() as conn:
        work_account = await conn.fetchrow(
            "SELECT * FROM work_accounts WHERE user_id = $1 LIMIT 1",
            user_id
        )
    
    if not work_account:
        # Проверяем системные команды без авторизации
        if command in ['.помощь', '.прайс', '.наличие', '.купить']:
            pass
        else:
            return
    
    # Системные команды с возможностью редактирования
    if command == '.наличие':
        async with db_pool.acquire() as conn:
            availability = await conn.fetch("""
                SELECT s.country, COUNT(*) as count, s.user_id, u.username
                FROM sale_accounts s 
                JOIN users u ON s.user_id = u.user_id
                WHERE s.is_sold = FALSE 
                GROUP BY s.country, s.user_id, u.username
            """)
        ...
        async with db_pool.acquire() as conn:
            availability = await conn.fetch("""
                SELECT country, COUNT(*) as count, MIN(price_rub) as price
                FROM sale_accounts 
                WHERE is_sold = FALSE 
                GROUP BY country
            """)
        ...
        for item in availability:
            text += f"<b>{item['country']}</b>: {item['count']} шт. - {item['price']}₽\n"
        ...
        # Проверяем, есть ли пользовательская команда .помощь
        async with db_pool.acquire() as conn:
            custom_cmd = await conn.fetchrow(
                "SELECT * FROM custom_commands WHERE command = '.помощь'"
            )
        
        if custom_cmd:
            await message.answer(custom_cmd['response'], parse_mode=ParseMode.HTML)
        else:
            await message.answer(
        ...
        if len(parts) < 2:
            await message.answer(
                '<tg-emoji emoji-id="5870657884844462243">❌</tg-emoji> Укажите страну: <code>.покупка Россия</code>',
                parse_mode=ParseMode.HTML
            )
            return
        
        country = parts[1].capitalize()
        buyer_id = message.from_user.id
        
        async with db_pool.acquire() as conn:
            available = await conn.fetchrow("""
                SELECT s.user_id, COUNT(*) as count, MIN(s.price_rub) as price
                FROM sale_accounts s 
                JOIN users u ON s.user_id = u.user_id
            ...
            invoice = await create_invoice(price, seller['crypto_token'], f"Покупка аккаунта {country}")
        ...
        # Проверяем пользовательскую команду
        async with db_pool.acquire() as conn:
            custom_cmd = await conn.fetchrow(
                "SELECT * FROM custom_commands WHERE command = '.прайс' AND user_id IN (SELECT user_id FROM sale_accounts WHERE is_sold = FALSE)"
            )
        
        if custom_cmd:
            await message.answer(custom_cmd['response'], parse_mode=ParseMode.HTML)
        else:
            # Показываем прайс-лист из базы
            async with db_pool.acquire() as conn:
                sellers = await conn.fetch("""
                    SELECT DISTINCT s.user_id, u.price_list
                    FROM sale_accounts s
                    JOIN users u ON s.user_id = u.user_id
                    WHERE s.is_sold = FALSE
                """)
            ...
            import json
            text = '<b><tg-emoji emoji-id="5904462880941545555">🪙</tg-emoji> Прайс-лист:</b>\n\n'
            shown_countries = set()
            
            for seller in sellers:
                if seller['price_list']:
                    price_list = json.loads(seller['price_list'])
                    for country, price in price_list.items():
...
# --- Обработка приветственного сообщения от рабочих аккаунтов ---
@dp.message(F.text & ~F.text.startswith('.') & ~F.text.startswith('/'))
async def handle_message_to_work_account(message: Message):
    # Проверяем, есть ли у пользователя рабочие аккаунты
    # Если это владелец бота, не отвечаем (чтобы не было конфликта с меню)
    async with db_pool.acquire() as conn:
        work_account = await conn.fetchrow(
            "SELECT * FROM work_accounts WHERE user_id = $1 LIMIT 1",
    ...
    # Проверяем, не является ли это сообщение от самого владельца фермы
    # (чтобы не отвечать на свои же сообщения в меню)
    if message.text in ["Менеджер аккаунтов", "Для продажи", "Редактирование", "Профиль"]:
        return
    
    # Получаем приветствие владельца фермы
    async with db_pool.acquire() as conn:
        # Находим всех пользователей, у которых есть рабочие аккаунты
        owners = await conn.fetch("""
            SELECT DISTINCT u.user_id, u.greeting_message 
            FROM users u 
            JOIN work_accounts w ON u.user_id = w.user_id
        """)
    
    # Отправляем приветствие от каждого владельца фермы
    for owner in owners:
        if owner['greeting_message']:
            ...
            # Отправляем приветствие (только один раз, от первого найденного)
